...
class StargazerCore:
    ...
    async def compute_star_neighbours(
        ...
        async def github_api_get_stargazers_repo_plus_return_stargazer_name(stargazer_name: str) -> tuple[str, Sequence[str]]:  # TODO: refactor
            return stargazer_name, await self.github_api.get_stargazer_repos(stargazer_name)

        repo_stargazers = await self.github_api.get_stargazers_of_repo(user_name, repo_name)
        for stargazer_name, stargazer_repos in await asyncio.gather(
            *(
                github_api_get_stargazers_repo_plus_return_stargazer_name(stargazer_name)
...
if __name__ == "__main__":
    async def main():
        ...
        # pp(await core.compute_star_neighbours("astariul", "github-hosted-pypi"))  # noqa: T203
        # pp(await core.compute_star_neighbours("msqd", "harp"))  # noqa: T203
        # pp(await core.compute_star_neighbours("meetup-python-grenoble", "meetup-python-grenoble.github.io"))  # noqa: T203
        # pp(await core.compute_star_neighbours("Lenormju", "talk-et-cfp"))  # noqa: T203
        print(await core.github_api.get_rate_limit_core_remaining())  # noqa: T201
